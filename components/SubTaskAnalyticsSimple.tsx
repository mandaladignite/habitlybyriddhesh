...
export default function SubTaskAnalytics({
  ...
  const calculateAnalytics = () => {
    ...
    // Filter data by time range and habit
    const filteredData = filterDataByTimeRange(progressData, timeRange)
    const filteredByHabit = selectedHabit === 'all' 
      ? filteredData 
      : filteredData.filter(p => p.habitId.toString() === selectedHabit)

    // Bottleneck Detection
    const bottlenecks = detectBottlenecks(subTasks, subTaskLogs, filteredByHabit)
    ...
    // Pattern Detection
    const patterns = detectPatterns(subTasks, subTaskLogs, filteredByHabit)
    ...
    // Recommendations
    const recommendations = generateRecommendations(habits, subTasks, filteredByHabit)
    ...
    // Achievements
    const achievements = detectAchievements(habits, filteredByHabit)
  ...
  const getOverallStats = () => {
    ...
    const totalHabits = selectedHabit === 'all' 
      ? habits.filter(h => h.hasSubTasks).length
      : habits.filter(h => h._id.toString() === selectedHabit && h.hasSubTasks).length
    
    const totalSubTasks = selectedHabit === 'all' 
      ? subTasks.length
      : subTasks.filter(st => st.habitId.toString() === selectedHabit).length
    
    const avgCompletion = filteredByHabit.length > 0 
      ? filteredByHabit.reduce((sum, p) => sum + p.completionPercentage, 0) / filteredByHabit.length 
      : 0
    
    const mostActiveDay = getMostActiveDay(filteredByHabit)
